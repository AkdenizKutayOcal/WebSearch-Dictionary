...
Title: Task7b
Description: Improved Version of Task5 (Explained in Report)
*/
...
int main()
{
    ...
    FILE *output;
    output = fopen("dictTask7b.txt", "w");

    printTrie(rootList[0], str, 0, output);
    fclose(output);

    clock_t end = clock();
    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
    printf("Task7b took %f seconds to execute.\n", time_spent);
}
...
void mergeDictionaries(struct TrieNode *root, unsigned char *str, int level)
{
    ...
    {
        str[level] = '\0';
        insert(rootList[0], str, (root->frequency));
    }
